...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_WIDTH, "LearnOpenGL", NULL, NULL);//宽度，高度标题
	if (window == NULL)
	{
	...
	//
	//
	Planet earth(0, 0, 0, 1, "Source/earth.jpg");
	//
	//
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glm::mat4 transform;
		transform = glm::rotate(transform, glm::radians(currentFrame * 10), glm::vec3(0.0f, 1.0f, 0.0f));
		// camera/view transformation
		glm::mat4 view = camera.GetViewMatrix();
		earth.flush(&projection, &transform, &view);
